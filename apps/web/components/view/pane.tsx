...
export function ViewPane({ viewId }: { viewId: string }) {
    ...
    const handleContextMenuMatch = (e: ReactMouseEvent) => {
        const nav = (e.target as HTMLElement).closest(
            ".i-tab-nav",
        ) as HTMLElement | null;
        if (!nav) return false;

        const index = Array.from(
            nav.parentElement!.querySelectorAll(".i-tab-nav"),
        ).indexOf(nav);
    ...
    return (
        ...
        >
            <div className={css.pane} onClick={() => setActiveView(viewId)}>
                {view.tabs.length > 0 && (
                    <PaneTabs
                        viewId={viewId}
                        tabs={view.tabs}
                        activeTabId={view.activeTabId ?? undefined}
                    />
                )}
            </div>
        </Popup>
